...
class Ui_Form(QtWidgets.QWidget):
    ...
    def select_folder_clicked(self):
        ...
        if self.foldername is None or self.foldername=='':return

        print(self.foldername)
        imgs_path =[]
        out_list=[]
        for i in os.listdir(self.foldername):
            if i.find('jpg')>0 or i.find('png')>0 or i.find("jpeg")>0:
                imgs_path.append(self.foldername+'/'+i)
        print(imgs_path)

        for i in imgs_path:
            if i.find('jpg')>0:        
               out_list.append(i.replace('.jpg', '_out.jpg'))
            if i.find('png')>0:        
                out_list.append(i.replace('.png', '_out.png'))
            if i.find('jpeg')>0:        
                out_list.append(i.replace('.jpeg', '_out.jpeg'))
        ...
        for i in range(0,len(imgs_path)):
            ...
            if str(type(sized)).find('float')>0:
                h = int(Img.height/sized)
                w = int(Img.width/sized)
                     # 处理jpg
                if imgs_path[i][-3:] == 'jpg':
                    # print(h,w)
            ...
            elif str(type(sized)).find('tuple')>0:
                h = sized[1]
                w = sized[0]
                     # 处理jpg
                if imgs_path[i][-3:] == 'jpg':
